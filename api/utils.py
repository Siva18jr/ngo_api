...
from .serializers import UserSerializer, ActivitySerializer, ProjectSerializer, ActivityImagesSerializer, WorkSerializer, AmountSerializer, FoodSerializer, DonationSerializer
...
def getPosts(request):
    ...
    ngo = request.query_params.get('ngo')

    posts = Posts.objects.filter(ngo=ngo)
    serializer = ActivitySerializer(posts, many=True)
...
def getProjects(request):
    ...
    projects = Projects.objects.filter(ngo=ngo)
...
def getAmountEntries(request):
    ...
    amount = Amount.objects.filter(ngo=ngo)
...
def addFood(request):
    ...
    ngo = request.data['ngo']

    if scheduled.lower() == 'all':
        if Food.objects.filter(date=date, ngo=ngo).exists() is True:
            return Response({
                'data' : {},
                'status' : False,
        ...
        if Food.objects.filter(scheduled=scheduled, date=date, ngo=ngo).exists() is True:
            return Response({
                'data' : {},
                'status' : False,
...
def getFoodData(request):
    ...
    food = Food.objects.filter(ngo=ngo)
...
def getDonations(request):
    ...
    donation = Donation.objects.filter(ngo=ngo)
...
def getAttendance(request):
    ...
    if type == 'date':
        ...
        users = AppUsers.objects.filter(ngo=ngo)
        ...
        user = AppUsers.objects.get(username=user, ngo=ngo)
...
def getDonationGraph(request):
    ...
    if duration == '6':
        ...
        lastSixMonthsData = Donation.objects.filter(created__gte=sixMonthsAgo, ngo=ngo)
        ...
        oneYearAgoData = Donation.objects.filter(created__gte=oneYearAgo, ngo=ngo)
        ...
        fiveYearsAgoData = Donation.objects.filter(created__gte=fiveYeasrAgo, ngo=ngo)
...
def getFoodListByDate(request):
    ...
    food = Food.objects.filter(date=date, ngo=ngo)
    serializer = FoodSerializer(food, many=True)
...
def getProjectGraph(request):
    ...
    if duration == 'Last Month':
        ...
        lastMonthAmount = Amount.objects.filter(created__gte=oneMonthAgo, ngo=ngo)
        lastMonthData = Projects.objects.filter(created__gte=oneMonthAgo, ngo=ngo)
        ...
        total_value = Amount.objects.filter(ngo=ngo).aggregate(total=Sum('spentAmount'))['total']
        categories = Amount.objects.filter(ngo=ngo).values('projectName').annotate(total_spent=Sum('spentAmount'))
        ...
        lastSixMonthsAmount = Amount.objects.filter(created__gte=sixMonthsAgo, ngo=ngo)
        lastSixMonthsData = Projects.objects.filter(created__gte=sixMonthsAgo, ngo=ngo)
